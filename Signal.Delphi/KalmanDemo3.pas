...
uses SignalUtils, StringVar, AbstractMtxVec;

{$R *.FMX}

{$IF (Defined(ANDROID) or Defined(IOS)) and Defined(ARITH_USE_LIBM)}
...
var i: integer;
    P, xhat: Matrix;
begin
    KalmanFilter.Q[0,0] := qEdit.FloatPosition; //-5; // # process variance
    KalmanFilter.R[0,0] := rEdit.FloatPosition; //-5; // # process variance
...
begin
    ...
    n_iter := 1000;
    xV.Size(0); //required for iOS Simulator only
    Tone(xV, n_iter, 8/1000, 0, 1);            //generate process without noise
    tmp := xV + RandGauss(n_iter,mvDouble,0,sigmaw);  //get process (plant) with noise
    Zv := tmp + RandGauss(n_iter,mvDouble,0,sigmav);  //get measurement with noise ( = plant + plant noise + measurement noise)
...
//    KalmanFilter.x.Size(2,1);
    ...
    rEdit.FloatPosition := sqr(sigmav);
    qEdit.FloatPosition := sqr(sigmaw);
end;
