...
uses
  ...
  Basic_Chart,
  MtxVec, Math387,
  FMXTee.Series, FMXTee.Engine, FMX.ListBox, FMX.Layouts, FMX.Memo,
  FMX.Controls, FMXTee.Procs, FMXTee.Chart, FMX.Types, FMX.Dialogs,
...
Uses FmxMtxVecTee, FmxMtxVecEdit, RegModels, Polynoms;
...
begin
  ...
  begin
    ...
    Add(' function PowerEval(B: Array of TSample; X: TSample): TSample;');
    Add(' begin');
    ...
    Add('       if (X.Values[i] > 0) and (Y.Values[i] > 0) then');
    Add('       begin');
    Add('         { linearize equation }');
    Add('         X1.Values[n] := Ln(X.Values[i]);');
    Add('         Y1.Values[n] := Ln(Y.Values[i]);');
    Add('         W1.Values[n] := Sqr(Y.Values[i]);');
    Add('         if Weights <> nil then W1.Values[n] := W1.Values[n]*Weights.Values[i];');
    Add('         Inc(n);');
    Add('       end;');
    Add('     X1.Resize(n);');
    Add('     Y1.Resize(n);');
...
begin
  ...
  0:  begin { Y=B*Exp[A*X] }
        ExpFit(B,X,Y,nil);
        MsgText := 'Y='+FormatFloat('0.00;-0.00)',B.Values[0])+'*'+
                   'Exp['+FormatFloat('0.00;-0.00',B.Values[1])+'*X]';
        B.CopyToArray(bvals);
  ...
  1:  begin { Y=k*X+n }
        ...
        MsgText := 'Y='+FormatFloat('0.00',B.Values[1])+'*'+
                   'X '+FormatFloat('+0.00;-0.00',B.Values[0]);
        B.CopyToArray(bvals);
  ...
  5:  begin { Logistic fit with constant term }
        ...
        MsgText := 'Y='+FormatFloat('0.00',B.Values[0]);
        MsgText := MsgText +'+['+FormatFloat('0.00',B.Values[1])+FormatFloat('-0.00;+0.00',B.Values[0])+']/';
        MsgText := MsgText+'[1+Exp('+FormatFloat('-0.00;+0.00',B.Values[2])+'x '+FormatFloat('+0.00;-0.00',B.Values[3])+']';
      end;
  6:  begin { Y=A+B*Ln(X) }
        LnFit(B,X,Y,nil);
        LnEval(B,X,YCalc);
  ...
  Label2.Text := 'RSS : ' + FormatSample('0.0000',Residuals.SumOfSquares);
  { update chart }
