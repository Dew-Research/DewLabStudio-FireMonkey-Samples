...
uses
  ...
  FMX.Header,
  Basic_Wizard, MtxVec,
  Statistics, Math387, FMX.Layouts, FMX.Memo, FMX.TabControl, FMX.Controls,
  FMX.Types, FMX.ScrollBox, FMX.Controls.Presentation;
...
type
  ...
  private
    ...
    Signif, Alpha: TSample;
    hRes: THypothesisResult;
    T2Total : TSample;
    radio1ItemIndex, radio2ItemIndex: integer;
...
begin
  ...
  if PageControl.ActiveTab = tsInputData then btnNext.Enabled := ((X1.Length>0) and (TwoSampleTest=false)) or ((X1.Cols>0) and (X1.Cols=X2.Cols));
end;
...
begin
  ...
  try
    ...
    if Assigned(ameans) then bmeans.Size(ameans.Length-1);
    c := 0;
    for j := 0 to amtx.Cols -1 do
      ...
      begin
        v1.GetCol(amtx,j);
        bmtx.SetCol(v1,c);
        if Assigned(ameans) then bmeans.Values[c] := ameans.Values[j];
        c := c + 1;
      end;
  finally
    FreeIt(v1);
...
procedure TfrmHotellingWizard.RadioButton1Change(Sender: TObject);
begin
  if (Sender = RadioButton1) and RadioButton1.IsChecked then Radio2ItemIndex := 0;
  if (Sender = RadioButton2) and RadioButton2.IsChecked then Radio2ItemIndex := 1;
  RadioGroup2Click(nil);
end;

procedure TfrmHotellingWizard.RadioButton3Change(Sender: TObject);
begin
    if (Sender = RadioButton3) and RadioButton3.IsChecked then Radio1ItemIndex := 0;
    if (Sender = RadioButton4) and RadioButton4.IsChecked then Radio1ItemIndex := 1;
    RadioGroup1Click(nil);
end;

procedure TfrmHotellingWizard.RadioGroup1Click(Sender: TObject);
begin
  TwoSampleTest := Radio1ItemIndex = 1;
...
begin
  UseMean := Radio2ItemIndex = 1;
  btnEditMeans.Visible := UseMean;
end;

procedure TfrmHotellingWizard.UpdateData;
begin
  UpdateDisplay;
end;
...
begin
  if X1.Length > 0 then
  begin
    lblSamples1.Text := 'Samples:'+IntToStr(X1.Rows);
    lblVariables1.Text := 'Variables:'+IntToStr(X1.Cols);
  end;

  if X1.Length > 0 then
  begin
    lblSamples2.Text := 'Samples:'+IntToStr(X2.Rows);
    lblVariables2.Text := 'Variables:'+IntToStr(X2.Cols);
  end;
