...
uses
  ...
  FMX.Header,
  Basic_Chart,
  MtxVec, FmxMtxVecTee, StatControlCharts, Math387, FMXTee.Engine, FMXTee.Series,
  FMX.Layouts, FMX.Memo, FMX.Controls, FMXTee.Procs, FMXTee.Chart, FMX.Types, FMXTee.Canvas;
...
function TfrmLevey.Series1GetPointerStyle(Sender: TChartSeries;
  ValueIndex: Integer): TSeriesPointerStyle;
begin
  if outcont.Values[ValueIndex]> 0 then Result := psCross else Result := psDownTriangle;
end;
...
begin
  MarkText := EncodeRules(Trunc(outcont.Values[ValueIndex]));
end;
...
begin
  ...
  if CheckBoxMean.IsChecked then
  begin
    ypos := Chart1.Axes.Left.CalcPosValue(m);
    Chart1.Canvas.Pen.Width := 1;
    Chart1.Canvas.Pen.Style := TPenStyle.psDot;
  ...
  if CheckBoxSigma.IsChecked then
  begin
    Chart1.Canvas.Pen.Width := 1;
    Chart1.Canvas.Pen.Style := psDashDot;
    // 1 sigma lines
    Chart1.Canvas.Pen.Color := TAlphaColors.Green;
    ...
    Chart1.Canvas.Line(Chart1.ChartRect.Left,ypos,Chart1.ChartRect.Right,ypos);
    // 2 sigma lines
    Chart1.Canvas.Pen.Color := TAlphaColors.Yellow;
    ...
    Chart1.Canvas.Line(Chart1.ChartRect.Left,ypos,Chart1.ChartRect.Right,ypos);
    // 3 sigma lines
    Chart1.Canvas.Pen.Color := TAlphaColors.Red;
