...
uses
  ...
  MtxVec,
  Math387,
  Statistics, FMX.Controls, FMX.Layouts, FMX.Memo, FMX.Types, FMX.ScrollBox,
...
begin
  ...
  if (checkBox1.IsChecked) then ReliabilityReport;
  if (checkBox2.IsChecked) then CountDistReport;
  if (checkBox3.IsChecked) then DetailsReport;
  if (checkBox4.IsChecked) then CorrReport;
  if (checkBox5.IsChecked) then CovReport;
end;

procedure TfrmItemAnalysis.UpdateGUI;
begin
  Label1.Text := 'Rows: '+IntToStr(Data.Rows);
  Label2.Text := 'Items: ' +IntToStr(Data.Cols);
end;
...
//  RichEdit1.SelAttributes.Style := [fsBold,fsUnderline];
  ...
  try
    Unique(Data,vars);
    st := 'Variable'+#9;
    for i := 0 to vars.Length -1 do st := st + FormatSample('0.0',vars.Values[i])+#9;
//    RichEdit1.SelAttributes.Color := clBlue;
    RichEdit1.Lines.Add(st);
    ...
    begin
      ...
      begin
        wrk2.FindAndGather(wrk1,'=',vars.Values[j]);
    ...
    for j := 0 to vars.Length -1 do
    begin
      wrk2.FindAndGather(Data,'=',vars.Values[j]);
      st := st + IntToStr(wrk2.Length)+#9;
    end;
...
begin
  ...
  begin
    ...
    wrk1.FindAndGather(ItemData,'=',vars.Values[i]);
    total := total + wrk1.Length;
    st := FormatSample('0.0',vars.Values[i])+#9 +IntToStr(wrk1.Length)+#9;
