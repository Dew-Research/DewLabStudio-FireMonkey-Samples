...
uses
  ...
  FMX.Forms,
  FMX.Header,
  Basic_Chart,MtxVec, Math387, Statistics, FMXTee.Series, FMXTee.Engine,
  FMXTee.Functions.Stats, FMXTee.Procs, FMXTee.Chart, FMX.Edit, FMX.ListBox,
  FMX.StdCtrls, FMX.Layouts, FMX.Memo, FMX.Controls, FMX.Types,
...
Uses Probabilities, StatRandom, FmxMtxVecTee, MtxVecUtils;
...
begin
  ...
  Numbins := 25;
  BinEdit.Text := IntToStr(NumBins);
  Alpha := 0.05;
  AlphaEdit.Text := SampleToStr(Alpha);
...
procedure TfrmGOFChi2.BinEditChange(Sender: TObject);
begin
  try
    NumBins := StrToInt(BinEdit.Text);
  except
    NumBins := 25;
    BinEdit.Text := IntToStr(NumBins);
  end;
...
procedure TfrmGOFChi2.AlphaEditChange(Sender: TObject);
begin
  try
    Alpha := StrToSample(AlphaEdit.Text);
  except
    Alpha := 0.05;
    AlphaEdit.Text := SampleToStr(Alpha);
  end;
