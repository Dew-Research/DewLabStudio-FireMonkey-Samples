...
uses
  ...
  MtxVec,
  MtxBaseComp,
  FMXTee.Commander, FMX.Layouts, FMX.Memo, FMX.Edit,
...
Uses FmxMtxVecEdit, Math387, Statistics, AbstractMtxVec,
     FmxMtxVecTee, StatSeries, Probabilities, StatProbPlots,
     Regress, MtxExpr
     {$IFDEF FIREMK}
...
//     SelAttributes.Color := clBlue;
  ...
  //   SelAttributes.Color := clBlue;
     ...
     begin
       ...
       LineText := LineText + FormatFloat(FormatString,BStdDev.Values[i])+chr(9);
       LineText := LineText + FormatFloat(FormatString,TVals.Values[i])+chr(9);
       LineText := LineText + FormatFloat(FormatString,PVals.Values[i])+chr(9);
       LineText := LineText + FormatFloat(FormatString,BConfInt.Values[i,0])+chr(9);
       LineText := LineText + FormatFloat(FormatString,BConfInt.Values[i,1]);
       Lines.Add(LineText)
     end;
     Lines.Add(Chr(13));
  end;
...
//    SelAttributes.Color := clBlue;
  ...
  { regression coefficients }
    ...
    with MtxMulLinReg do
    begin
      LineText := FormatFloat(FormatString,Y.Values[i])+chr(9);
      LineText := LineText + FormatFloat(FormatString,RegressResult.YCalc.Values[i])+chr(9);
      LineText := LineText + FormatFloat(FormatString,RegressResult.Residuals.Values[i]);
      Lines.Add(LineText)
    end;
    Lines.Add(Chr(13));
  end;
...
//    RichEdit.SelAttributes.Size := 16;
    ...
    RichEdit.Lines.Add(Chr(13));
    RichEdit.Lines.Add(Chr(13));
    if ChkBoxRegCoeff.IsChecked then RegCoeffReport;
    if ChkBoxRes.IsChecked then ResidualReport;
    if ChkBoxNormality.IsChecked then NormalityReport;
    if ChkBoxRegStats.IsChecked then BasicRegStatReport;
    if ChkBoxDescStat.IsChecked then DescStatReport;
    if ChkBoxCorr.IsChecked then CorrMtxReport;
    if ChkBoxSerCorr.IsChecked then SerCorrReport;
  finally
...
//    Chart1.Series[0].SeriesColor := clBlue;
    ...
    With MtxMulLinReg.RegressResult do
    begin
      tmpVec1.Size(Residuals.Length -1);
      tmpVec2.Size(Residuals.Length -1);
...
procedure TfrmMulLinRegWiz.UpdateData;
begin
  { initialize page }
...
begin
  MtxMulLinReg.UseWeights := ChkBoxUseWeights.IsChecked;
  RefreshWizardControls;
end;

procedure TfrmMulLinRegWiz.ChkBoxRemoveIntClick(Sender: TObject);
begin
  MtxMulLinReg.Constant := Not (ChkBoxRemoveInt.IsChecked);
  RefreshWizardControls;
end;
...
begin
  inherited;
  if PageControl.ActiveTab = tsInputData then btnNext.Enabled := MtxMulLinReg.ValidSystem
