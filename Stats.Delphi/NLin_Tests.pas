...
uses
  ...
  FMX.Header,
  Basic_Form,
  StatTools, FMX.Edit, FMX.ListBox, FMX.Controls, FMX.Layouts, FMX.Memo,
  FMX.Types, FMX.Controls.Presentation, FMX.ScrollBox, FMX.Memo.Types;



type
  TfrmNonLinTest = class(TfrmBasic)
    Panel2: TPanel;
    Label1: TLabel;
    ComboBox1: TComboBox;
    Label2: TLabel;
  ...
  private
    ...
    procedure WriteToLog(Elapsed: DWord);
...
var
  frmNonLinTest: TfrmNonLinTest;

implementation

Uses Optimization, FmxMtxVecTee, Math387, FmxMtxVecEdit, LogMemo, MtxVec,  MtxIntDiff, AbstractMtxVec, MtxVecUtils
     {$IFNDEF FIREMK}
     ,Types
     {$ENDIF}
     ;
...
function Eckerle4(const B: TVec; X: TSample): TSample;
begin
  Result := B[0]/B[1] * Exp(-0.5*Sqr((X-B[2])/B[1]));
end;

function Thurber(const B: TVec; X: TSample): TSample;
begin
  Result := (B[0] + B[1]*X + B[2]*Sqr(X) + B[3]*IntPower(X,3)) /
            (1.0 + B[4]*X + B[5]*Sqr(X) + B[6]*IntPower(X,3));
end;

function Rat42(const B: TVec; X: TSample): TSample;
begin
  Result := B[0] / (1.0 + Exp(B[1]-B[2]*X));
end;

function Misra1c(const B: TVec; X: TSample): TSample;
begin
  Result := B[0]*(1-Power(1+2*B[1]*X,-0.5));
end;

function MGH09(const B: TVec; X: TSample): TSample;
begin
  Result := B[0]*(Sqr(X) + X*B[1]) / (Sqr(X) + X*B[2]+B[3]);
end;

function Chwirut2(const B: TVec; X: TSample): TSample;
begin
  Result := Exp(-B[0]*X)/(B[1]+B[2]*X);
end;

function MGH10(const B: TVec; X: TSample): TSample;
begin
  Result := B[0]*Exp(B[1]/(X+B[2]));
end;

function Roszman1(const B: TVec; X: TSample): TSample;
begin
  Result := B[0]-B[1]*X - ArcTan(B[2]/(X-B[3]))/PI;
end;

function Rat43(const B: TVec; X: TSample): TSample;
begin
  Result := B[0] / Power((1.0 + Exp(B[1]-B[2]*X)),1/B[3]);
end;

procedure TfrmNonLinTest.ComboBox1Change(Sender: TObject);
begin
  MtxNonLinReg.OptMethod := TOptMethod(ComboBox1.ItemIndex);
  SpeedButton1.Enabled := true;
end;

procedure TfrmNonLinTest.ComboBox2Change(Sender: TObject);
begin
...
procedure TfrmNonLinTest.FillData(Const Index: Integer);
begin
  if Index = 0 then { Eckerle4 }
  begin
    MtxNonLinReg.X.SetIt(false,[400.0, 405.0, 410.0, 415.0,
  ...
  begin
    MtxNonLinReg.X.SetIt(false,[-3.067, -2.981, -2.921, -2.912,
  ...
  begin
    MtxNonLinReg.X.SetIt(false,[77.6, 114.9, 141.1, 190.8, 239.9,
  ...
  begin
    MtxNonLinReg.X.SetIt(false,[4.000000E+00, 2.000000E+00, 1.000000E+00,
  ...
  begin
    MtxNonLinReg.X.SetIt(false,[0.50, 1.00, 1.75, 3.75, 5.75, 0.875, 2.25,
  ...
  begin
    MtxNonLinReg.X.SetIt(false,[-4868.68,-4868.09,-4867.41,-3375.19,-3373.14,-3372.03,
...
procedure TfrmNonLinTest.SpeedButton1Click(Sender: TObject);
var Elapsed: double;
begin
  SetHourGlassCursor;
  try
...
procedure TfrmNonLinTest.DisplayInfo(const Index: Integer);
begin
  Memo2.Lines.Clear;
  With Memo2.Lines do
    if Index = 0 then
...
procedure TfrmNonLinTest.Edit3Change(Sender: TObject);
begin
  try
...
begin
  inherited;
  MtxNonLinReg := TMtxNonLinReg.Create(nil);
...
procedure TfrmNonLinTest.Edit1Change(Sender: TObject);
begin
  try
...
procedure TfrmNonLinTest.WriteToLog(Elapsed: DWord);
var i: Integer;
    tmpStr: String;
begin
  case ComboBox2.ItemIndex of
...
procedure TfrmNonLinTest.Button1Click(Sender: TObject);
begin
  frmLogForm.Show;
end;
...
procedure TfrmNonLinTest.CheckBox1Click(Sender: TObject);
begin
  if CheckBox1.IsChecked then frmLogForm.Show
  else frmLogForm.Hide;
end;

procedure TfrmNonLinTest.ResetEstimates(const Index: Integer);
begin
  if Index = 0 then { Eckerle4 }
...
procedure TfrmNonLinTest.Edit4Change(Sender: TObject);
begin
  try
...
procedure TfrmNonLinTest.Edit2Change(Sender: TObject);
begin
  try
