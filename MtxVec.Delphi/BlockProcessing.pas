...
uses
  ...
  MtxExpr, AbstractMtxVec, Probabilities, Math387, FmxMtxComCtrls, MtxVec, PlatformHelpers,
  FMX.Layouts, System.Classes, FMX.ScrollBox, FMX.Controls.Presentation;
...
constructor TfrmBlockProc.Create(AOwner: TComponent);
begin
  inherited;
  FormCreate(Self);
end;

destructor TfrmBlockProc.Destroy;
begin
//  Res.Adopt(nil);
//  X.Adopt(nil);
//  tmp.Adopt(nil);
...
begin
  ...
  begin
       ...
       Label1.Text := 'Progress: ' + FormatFloat('0',(i+0.33)/Points*100) + '%';

       Res.SetZero;
       MaxwellNoBlock(Iters,a2[i]);
       Label1.Text := 'Progress: ' + FormatFloat('0',(i+0.66)/Points*100) + '%';

       Res.SetZero;
       a3[i] := 0;
       MaxwellBlock(Iters,a3[i]);
       Label1.Text := 'Progress: ' + FormatFloat('0',(i+0.99)/Points*100) + '%';
