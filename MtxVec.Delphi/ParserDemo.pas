...
begin
  ...
  aText := ExpressionEdit.Text;
  Insert(S, aText, ExpressionEdit.CaretPosition);
  ExpressionEdit.Text := aText;
end;
...
procedure TfrmParser.ExpressionEditKeyDown(Sender: TObject; var Key: Word;
  var KeyChar: Char; Shift: TShiftState);
begin
  inherited;
...
begin
  AddToEdit((Sender as TButton).Text);
end;
...
{  if (FunctionsCombo.ItemIndex >= 0) and (FunctionsCombo.ItemIndex < Maxstrings) then
    Helplabel.Text := Helpstrings[FunctionsCombo.ItemIndex]; }
end;

procedure TfrmParser.Button32Click(Sender: TObject);
begin
  AddToEdit(FunctionsCombo.Selected.Text);
end;

constructor TfrmParser.Create(AOwner: TComponent);
begin
  inherited;
  ...
  PointButton.Text := GetDecimalSeparator;
...
begin
  AddToEdit(ConstantsCombo.Selected.Text)
end;
